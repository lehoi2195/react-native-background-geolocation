...
if (!isInstalled.forIos()) {
  ...
  const missingBgModes = config.REQUIRED_BACKGROUND_MODES.filter(function (
    mode
  ) {
  ...
  if (!infoPlist.NSLocationAlwaysUsageDescription) {
    pListChanges.NSLocationAlwaysUsageDescription =
      "App requires background tracking";
  }
  if (!infoPlist.NSLocationAlwaysAndWhenInUseUsageDescription) {
    pListChanges.NSLocationAlwaysAndWhenInUseUsageDescription =
      "App requires background tracking";
  }
  ...
  if (!infoPlist.NSLocationWhenInUseUsageDescription) {
    pListChanges.NSLocationWhenInUseUsageDescription =
      "App requires background tracking";
  }
